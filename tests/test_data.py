    ...
    recipe = Recipe(
        ...
        ingredient_groups=[
            IngredientGroup(
                ...
                ingredients=[
                    Ingredient(
                        amount=Amount(factor=Decimal("2"), unit="cloves"), name="Garlic"
                    ),
                ],
                ingredient_groups=[
                    IngredientGroup(
...
class TestRecipeParser:
    @pytest.mark.parametrize(
        "testcase_file",
        glob.glob(
            os.path.join(os.path.dirname(__file__), "..", "testcases", "cases", "*.md")
        ),
    )
    ...
    def test_parse_amount(self, parser):
        ...
        assert parser.parse_amount("5 1/4 ml") == Amount(
            factor=Decimal("5.25"), unit="ml"
        )
        ...
        assert parser.parse_amount("3,2") == Amount(factor=Decimal("3.2"))
        assert parser.parse_amount("1 ½ cloves") == Amount(
            factor=Decimal("1.5"), unit="cloves"
        )
        assert parser.parse_amount("½ pieces") == Amount(
            factor=Decimal(".5"), unit="pieces"
        )
...
class TestRecipeSerializer:
    def test_serialize(self, serializer):
        testcase_folder = os.path.join(
            os.path.dirname(__file__), "..", "testcases", "cases"
        )
        with open(
            os.path.join(testcase_folder, "recipe.md"), "r", encoding="UTF-8"
        ) as f:
            expected_result = f.read()
        with open(
            os.path.join(testcase_folder, "recipe.json"), "r", encoding="UTF-8"
        ) as f:
    ...
    def test_serialize_amount(self, serializer):
        ...
        assert (
            serializer._serialize_amount(
                Amount(factor=Decimal("1") / Decimal("3")), rounding=2
            )
            == "0.33"
        )
        assert (
            serializer._serialize_amount(
                Amount(factor=Decimal("1") / Decimal("3")), rounding=4
            )
            == "0.3333"
        )
