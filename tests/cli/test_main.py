...
def test_export_links():
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            mock.patch(
                "sys.argv",
                ["", "--export-links", actual_output_dir_relative, input_file],
            )
        )
        stack.enter_context(mock.patch("sys.stdout", new_callable=StringIO))
        stack.enter_context(mock.patch("sys.stderr", new_callable=StringIO))
    ...
    files_to_compare = set(
        os.listdir(expected_output_dir) + os.listdir(actual_output_dir)
    )
    (_, mismatch, errors) = filecmp.cmpfiles(
        expected_output_dir, actual_output_dir, files_to_compare, shallow=False
    )
