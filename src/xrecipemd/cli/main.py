...
def _yield_completer(prefix, action, parser, parsed_args):
    try:
        ...
        return [
            RecipeSerializer._serialize_amount(Amount(parsed_yield.factor, a.unit))
            for a in r.yields
            if parsed_yield.unit is None
            or (a.unit is not None and a.unit.startswith(parsed_yield.unit))
        ]
...
def _process_scaling(r, args):
    ...
    if args.required_yield is not None:
        ...
        if multiply.unit is not None:
            print(
                f"A recipe can only be multiplied with a unitless amount",
                file=sys.stderr,
            )
            raise Exit()
...
def _make_missing_yield_warning(recipe: Recipe, required_yield: Amount):
    ...
    if recipe.yields:
        warning += " The following units can be used: " + ", ".join(
            f'"{y.unit}"' for y in recipe.yields if y.unit
        )
    return warning
...
def _export_links(r, args, recipe_url, parser, serializer):
    ...
    link_ingredients, ingr_to_recipe = _get_linked_recipes(
        r, recipe_url=recipe_url, parser=parser, flatten=True
    )
...
def _create_recipe_output(recipe, serializer, args):
    ...
    if args.title:
        ...
        return "\n".join(
            _ingredient_to_string(ingr, rounding=args.round)
...
def _get_flattened_recipe(
    recipe: Recipe,
    *,
    recipe_url: URL,
    parser: RecipeParser,
    exclude_urls: Set[URL] = frozenset(),
) -> Recipe:
    """Creates a new recipe with linked recipes recursively flattened"""

    exclude_urls |= {recipe_url}
    link_ingredients, ingr_to_recipe = _get_linked_recipes(
        recipe, recipe_url=recipe_url, parser=parser, exclude_urls=exclude_urls
    )
        ...
        try:
            ...
            if link_recipe.instructions:
                instruction_sections.append(
                    (
                        _link_ingredient_title(ingredient, link_recipe),
                        link_recipe.instructions,
                        False,
                    )
                )
    ...
    if len(instruction_sections) == 1 and instruction_sections[0][2]:
        ...
        for heading, body, is_main_instructions in instruction_sections:
            ...
            new_body = re.sub(
                r"^( {0,3})(#{1,5}.*)$", r"\1#\2", body, flags=re.MULTILINE
            )
...
def _get_linked_recipes(
    ...
    *,
    ...
    for ingredient in link_ingredients:
        try:
            ingr_to_recipe[ingredient] = _get_linked_recipe(
                ingredient,
...
def _get_linked_recipe(
    ...
    *,
    ...
    if url in exclude_urls:
        raise RuntimeError(
            f"""Skipping ingredient "{ingredient.name}" to prevent infinite recursion"""
        )
    ...
    if flatten:
        link_recipe = _get_flattened_recipe(
            link_recipe, recipe_url=url, parser=parser, exclude_urls=exclude_urls
        )
    ...
    if ingredient.amount:
        try:
            ...
            print(
                _make_missing_yield_warning(link_recipe, ingredient.amount),
                file=sys.stderr,
            )
...
def _create_flattened_substituted_ingredients(
    ingredient_list: IngredientList, ingr_to_recipe: Dict[Ingredient, Recipe]
) -> IngredientList:
    result_ingredients = []
    result_groups = []
...
parser.add_argument(
    "-v", "--version", action="version", version=f"%(prog)s ({recipemd.__version__})"
)

display_parser = parser.add_mutually_exclusive_group()
display_parser.add_argument(
    "-t", "--title", action="store_true", help="Display recipe title"
)
display_parser.add_argument(
    "-i", "--ingredients", action="store_true", help="Display recipe ingredients"
)
display_parser.add_argument(
    "-j", "--json", action="store_true", help="Display recipe as JSON"
)

parser.add_argument(
    "-r",
    "--round",
    type=lambda s: None if s.lower() == "no" else int(s),
    metavar="n",
    default=2,
...
scale_parser.add_argument(
    "-m", "--multiply", type=str, help="Multiply recipe by N", metavar="N"
)
scale_parser.add_argument(
    "-y",
    "--yield",
    type=str,
    help='Scale the recipe for yield Y, e.g. "5 servings"',
    metavar="Y",
    dest="required_yield",
