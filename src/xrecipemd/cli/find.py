...
def list_recipes(args):
    print_result(
        [path for recipe, path in get_filtered_recipes(args)], args.output_multicol
    )
    ...
    for path in glob.glob(os.path.join(args.folder, "**/*.md"), recursive=True):
        try:
            ...
            if not args.no_messages:
                print(
                    f"An error occurred, skipping {os.path.relpath(path, args.folder)}: {e.args[0]}",
                    file=sys.stderr,
                )
    return result
...
def print_columns(items, transpose=True):
    ...
    if transpose:
        ...
        matrix = [
            items[i : i + column_count] for i in range(0, len(items), column_count)
        ]
...
def create_filter_expr(filter_string) -> _FilterElement:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f'"{filter_string}" is not a valid filter: {e}'
        )
    except re.error as e:
        raise argparse.ArgumentTypeError(
            f'"{filter_string}" contains the regular expression "{e.pattern}": {e}'
        )


# parser is on module level for sphinx-autoprogram
parser = argparse.ArgumentParser(
    description="Find recipes, ingredients and units by filter expression"
)
...
parser_tags.add_argument(
    "-c", "--count", action="store_true", help="count number of uses per tag"
)
parser_tags.add_argument(
    "folder",
    type=dir_path,
...
parser_ingredients.add_argument(
    "-c", "--count", action="store_true", help="count number of uses per ingredient"
)
parser_ingredients.add_argument(
    "folder",
    type=dir_path,
...
parser_units.add_argument(
    "-c", "--count", action="store_true", help="count number of uses per unit"
)
parser_units.add_argument(
    "folder",
    type=dir_path,
